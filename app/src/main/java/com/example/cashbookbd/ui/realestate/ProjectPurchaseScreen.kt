...
/**
 * Project Purchase — the construction purchase invoice with Building in place
 * of Warehouse (this branch has no warehouses, and two location dropdowns side
 * by side would blur). Every product row carries its project and building; the
 * server writes one Purchase ledger line per building, keeps the stock, and
 * picks the voucher shape (cash / credit / partial) itself.
 */
@Composable
fun ProjectPurchaseScreen(
    navController: NavHostController,
    onLogout: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: ProjectPurchaseViewModel = viewModel(
        factory = ProjectPurchaseViewModel.provideFactory(androidx.compose.ui.platform.LocalContext.current),
    ),
) {
    ...
    AuthenticatedShell(
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            Column(
                ...
                Text(
                    text = "A purchase invoice whose every product row says which project — " +
                        "and building — the material is for. The ledger, stock and payment " +
                        "shapes are the server's business.",
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    PickerField(
                        ...
                        modifier = Modifier.weight(1f),
                        onClick = {
                            showInvoiceDatePicker(context, state.invoiceDate) { picked ->
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    AppTextField(
                        ...
                        label = "Paid",
                        caption = if (state.isCashSupplier) "A cash purchase is paid in full." else "",
                ...
                AppSelectDropdown(
                    label = "Building",
                    options = purchaseBuildingOptions(state.buildings),
                    selected = purchaseBuildingOptions(state.buildings).firstOrNull { it.id == state.buildingId },
                ...
                SearchableSelectDropdown(
                    selected = state.productId.takeIf { it.isNotBlank() }
                        ?.let { SelectorOption(id = it, label = state.productName) },
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    AppTextField(
                        ...
                        label = "Price",
                        caption = "Filled from the last purchase; change it if today differs.",
                        keyboardType = KeyboardType.Number,
                        modifier = Modifier.weight(1f),
                    )
                }
                ...
                ReportTable(
                    ...
                    data = state.rows,
                    noDataMessage = "No products yet — Add New puts the form above into the invoice.",
                    rowBackground = { row, _ ->
...
@Composable
private fun purchaseBuildingOptions(buildings: List<SelectorOption>): List<SelectorOption> =
    remember(buildings) {
        listOf(SelectorOption(id = "", label = "Whole project (no single building)")) + buildings
    }

@Composable
private fun purchaseColumns(
    editingRowKey: String?,
    onEdit: (String) -> Unit,
    onRemove: (String) -> Unit,
): List<ReportColumn<ProjectPurchaseLine>> {
...
private fun showInvoiceDatePicker(context: Context, current: String, onPicked: (String) -> Unit) {
